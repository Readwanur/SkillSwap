...
<div class="page-header">
    ...
    <span class="badge badge-info" style="padding:6px 14px; font-size:0.8rem;">Refund window:
        <?php echo REFUND_WINDOW_DAYS; ?> days</span>
</div>
...
function buildTabUrl($status, $sort, $order)
{
    ...
    if ($sort !== '')
        $q['sort'] = $sort;
    if ($order !== '')
        $q['order'] = strtolower($order);
    return '?' . http_build_query($q);
}
?>
<div class="stats-grid" style="margin-bottom:16px;">
    <div class="stat-card stat-card-accent" style="--accent: var(--primary); cursor:pointer;"
        onclick="location.href='<?php echo buildTabUrl('', $sort, $order); ?>'">
        <span class="stat-value"><?php echo $count_all; ?></span>
        <span class="stat-label">Total Sessions</span>
    </div>
    <div class="stat-card stat-card-accent" style="--accent: var(--warning); cursor:pointer;"
        onclick="location.href='<?php echo buildTabUrl('scheduled', $sort, $order); ?>'">
        <span class="stat-value"><?php echo $count_scheduled; ?></span>
        <span class="stat-label">Scheduled</span>
    </div>
    <div class="stat-card stat-card-accent" style="--accent: #2196F3; cursor:pointer;"
        onclick="location.href='<?php echo buildTabUrl('under-review', $sort, $order); ?>'">
        <span class="stat-value"><?php echo $count_under_review; ?></span>
        <span class="stat-label">Under Review</span>
    </div>
    <div class="stat-card stat-card-accent" style="--accent: var(--success); cursor:pointer;"
        onclick="location.href='<?php echo buildTabUrl('completed', $sort, $order); ?>'">
        <span class="stat-value"><?php echo $count_completed; ?></span>
        <span class="stat-label">Completed</span>
    </div>
    <div class="stat-card stat-card-accent" style="--accent: var(--danger); cursor:pointer;"
        onclick="location.href='<?php echo buildTabUrl('cancelled', $sort, $order); ?>'">
        <span class="stat-value"><?php echo $count_cancelled; ?></span>
        <span class="stat-label">Cancelled</span>
    </div>
    <div class="stat-card stat-card-accent" style="--accent: #9c27b0; cursor:pointer;"
        onclick="location.href='<?php echo buildTabUrl('refunded', $sort, $order); ?>'">
...
<?php if ($recent_refunds->num_rows > 0): ?>
    <div class="card mb-3" style="border-left: 4px solid #9c27b0;">
        <div class="card-header">
            <h3><i data-lucide="banknote" class="lucide-sm"></i> Recent Refunds</h3>
        </div>
        <div style="padding: 0 16px 12px;">
            <?php while ($ref = $recent_refunds->fetch_assoc()): ?>
                <div
                    ...
                    <span class="badge <?php echo $ref['type'] === 'full_refund' ? 'badge-danger' : 'badge-warning'; ?>"
...
<div class="card">
    <div class="card-header" style="flex-wrap:wrap; gap:8px;">
        ...
        <div class="admin-filter-tabs">
            <a href="<?php echo buildTabUrl('', $sort, $order); ?>"
                class="filter-tab <?php echo $status_filter === '' ? 'active' : ''; ?>">
                All Statuses
                <?php if ($count_all > 0): ?><span
                        class="admin-notif-badge"><?php echo $count_all; ?></span><?php endif; ?>
            </a>
            <a href="<?php echo buildTabUrl('scheduled', $sort, $order); ?>"
                class="filter-tab <?php echo $status_filter === 'scheduled' ? 'active' : ''; ?>">
                <i data-lucide="hourglass" class="lucide-sm"></i> Scheduled
                <?php if ($count_scheduled > 0): ?><span
                        class="admin-notif-badge"><?php echo $count_scheduled; ?></span><?php endif; ?>
            </a>
            <a href="<?php echo buildTabUrl('under-review', $sort, $order); ?>"
                class="filter-tab <?php echo $status_filter === 'under-review' ? 'active' : ''; ?>">
                <i data-lucide="search" class="lucide-sm"></i> Under Review
                <?php if ($count_under_review > 0): ?><span
                        class="admin-notif-badge"><?php echo $count_under_review; ?></span><?php endif; ?>
            </a>
            <a href="<?php echo buildTabUrl('completed', $sort, $order); ?>"
                class="filter-tab <?php echo $status_filter === 'completed' ? 'active' : ''; ?>">
                <i data-lucide="check-circle" class="lucide-sm"></i> Completed
                <?php if ($count_completed > 0): ?><span
                        class="admin-notif-badge"><?php echo $count_completed; ?></span><?php endif; ?>
            </a>
            <a href="<?php echo buildTabUrl('cancelled', $sort, $order); ?>"
                class="filter-tab <?php echo $status_filter === 'cancelled' ? 'active' : ''; ?>">
                <i data-lucide="x-circle" class="lucide-sm"></i> Cancelled
                <?php if ($count_cancelled > 0): ?><span
                        class="admin-notif-badge"><?php echo $count_cancelled; ?></span><?php endif; ?>
            </a>
            <a href="<?php echo buildTabUrl('refunded', $sort, $order); ?>"
                class="filter-tab <?php echo $status_filter === 'refunded' ? 'active' : ''; ?>">
                <i data-lucide="banknote" class="lucide-sm"></i> Refunded
                <?php if ($count_refunded > 0): ?><span
                        class="admin-notif-badge"><?php echo $count_refunded; ?></span><?php endif; ?>
            </a>
        </div>
    </div>
                ...
                <tbody>
                    <?php while ($d = $disputes->fetch_assoc()):
                        ...
                        <tr>
                            ...
                            <td style="font-size:0.82rem;"><?php echo date('M d, Y h:i A', strtotime($d['scheduled_time'])); ?>
                            </td>
                            ...
                            <td>
                                ...
                                <?php if ($d['submission_note']): ?>
                                    <br><small style="color:var(--text-muted); font-size:0.75rem;">Proof:
                                        "<?php echo htmlspecialchars($d['submission_note']); ?>"</small>
                                <?php endif; ?>
                            </td>
                            <td>
                                <?php if ($d['rating']): ?>
                                    <span title="<?php echo htmlspecialchars($d['comment'] ?? ''); ?>">
                                        <i data-lucide="star" class="lucide-sm" style="color: gold;"></i>
                                        <?php echo $d['rating']; ?>/5
                                    </span>
                                <?php else: ?>
                                    ...
                                    <div class="flex gap-1" style="flex-wrap:nowrap;">
                                        <form method="POST" style="display:inline;">
                                            ...
                                            <button type="submit" class="btn btn-sm btn-success"
                                                title="Force complete without credit transfer"><i data-lucide="check-circle"
                                                    class="lucide-sm"></i></button>
                                        </form>
                                        <form method="POST" style="display:inline;">
                                            <input type="hidden" name="session_id" value="<?php echo $d['session_id']; ?>">
                                            <input type="hidden" name="action" value="resolve_cancelled">
                                            <button type="submit" class="btn btn-sm btn-danger" title="Cancel session"><i
                                                    data-lucide="x" class="lucide-sm"></i></button>
                                        </form>
                                    </div>
                                <?php elseif ($d['status'] === 'under-review'): ?>
                                    <div class="flex gap-1" style="flex-wrap:nowrap;">
                                        <form method="POST" style="display:inline;">
                                            ...
                                            <button type="submit" class="btn btn-sm btn-success"
                                                title="Approve and Transfer Credits"><i data-lucide="check-circle"
                                                    class="lucide-sm"></i> Approve</button>
                                        </form>
                                        <form method="POST" style="display:inline;">
                                            <input type="hidden" name="session_id" value="<?php echo $d['session_id']; ?>">
                                            <input type="hidden" name="action" value="reject_proof">
                                            <button type="submit" class="btn btn-sm btn-warning" title="Reject Proof"><i
                                                    data-lucide="corner-down-left" class="lucide-sm"></i> Reject</button>
                                        </form>
                                    </div>
                                <?php elseif ($d['status'] === 'completed'): ?>
                                    <?php if ($refund_eligible): ?>
                                        <button type="button" class="btn btn-sm btn-danger"
                                            ...
                                            title="Issue refund"><i data-lucide="banknote" class="lucide-sm"></i> Refund</button>
                                    <?php else: ?>
                                        <span style="font-size:0.72rem; color:var(--text-muted);"
                                            title="Refund window (<?php echo REFUND_WINDOW_DAYS; ?> days) has expired"><i
                                                data-lucide="clock" class="lucide-sm"></i> Expired</span>
                                    <?php endif; ?>
                                <?php elseif ($d['status'] === 'refunded'): ?>
                                    <span class="badge"
                                        style="background:rgba(156,39,176,0.1); color:#9c27b0; font-size:0.7rem;">Refunded</span>
                                <?php else: ?>
                                    <span style="color:var(--text-muted);">—</span>
                                <?php endif; ?>
                            </td>
    ...
    <?php else: ?>
        <div class="empty-state">
            <div class="icon" style="color: var(--text-muted);"><i data-lucide="message-square"
...
<div class="modal-overlay" id="refund-modal">
    <div class="modal">
        <div class="modal-header">
            <h3><i data-lucide="banknote" class="lucide-sm"></i> Issue Refund</h3>
            <button class="modal-close"
                onclick="document.getElementById('refund-modal').classList.remove('active')">&times;</button>
        </div>
        <form method="POST">
            ...
            <div style="background:var(--bg-hover); border-radius:var(--radius-sm); padding:12px; margin-bottom:16px;">
                <div class="grid-2" style="gap:8px;">
                    <div><small style="color:var(--text-muted);">Session</small><br><strong
                            id="refund-session-label">#—</strong></div>
                    <div><small style="color:var(--text-muted);">Max Amount</small><br><strong id="refund-max-label">—
                            TC</strong></div>
                    <div><small style="color:var(--text-muted);">Requester (receives)</small><br><span
                            id="refund-requester-label">—</span></div>
                    <div><small style="color:var(--text-muted);">Provider (deducted)</small><br><span
                            id="refund-provider-label">—</span></div>
                </div>
                <div style="margin-top:8px; font-size:0.78rem; color:var(--text-muted);">
                    <i data-lucide="clock" class="lucide-sm"></i> <span id="refund-days-label">—</span> days remaining
                    in refund window
                </div>
            </div>
            ...
            <div class="form-group">
                ...
                <div class="flex gap-1" style="align-items:center;">
                    <input type="number" name="refund_amount" id="refund-amount" class="form-control" min="0.01"
                        step="0.01" required style="flex:1;">
                    <button type="button" class="btn btn-sm btn-secondary"
                        onclick="document.getElementById('refund-amount').value = document.getElementById('refund-amount').max"
                        title="Set to full amount">Full</button>
                </div>
                <small style="color:var(--text-muted); font-size:0.72rem;">Enter the full amount for a complete refund,
                    or a smaller amount for a partial refund.</small>
            </div>
            ...
            <div class="form-group">
                ...
                <textarea name="refund_reason" id="refund-reason-text" class="form-control mt-1" rows="2"
                    placeholder="Describe the refund reason..." required></textarea>
            </div>

            <!-- Impact Warning -->
            <div class="alert alert-warning" style="font-size:0.82rem; margin-bottom:16px;">
                <strong><i data-lucide="alert-triangle" class="lucide-sm" style="color: var(--warning);"></i> Refund
                    Impact:</strong>
            ...
            <div class="flex gap-1" style="justify-content:flex-end;">
                <button type="button" class="btn btn-secondary"
                    onclick="document.getElementById('refund-modal').classList.remove('active')">Cancel</button>
